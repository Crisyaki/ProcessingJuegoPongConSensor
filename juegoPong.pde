...
Serial myPort;
String val; // Data received from the s e ri a l port
String portName;
int HEIGHT = 1000;
float floty;
...
void setup(){
  ...
  portName = Serial.list()[0]; //change the 0 to a 1 or 2 e tc . to match your port
...
float sensorProximidad(){
  float floty = 500; //MODIFICAR AQUI!!!!!!
  ...
  if (val != null) {
    try {
      ...
      if (out > 64 && out < 480) floty = (map(out, 64, 480, 0, 1000));
      println(floty);
    } catch (Exception e) {
      println(e);
    }
